...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
		...
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            ...
            //Charles fix image and title frame
            CGRect imageRect = CGRectMake(imageXOffset, 5, imageWidth, imageHeight);
            CGRect textRect = CGRectMake(textXOffset, yOffset + imageHeight / 2 + 1, textWidth, stringHeight);
            //end Charles
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(enlargeRect, touchLocation)) {
        ...
        //trongpv
        [self.delegate hmSegmentControlWillSelectedIndex:(int)segment];
        // end trongpv
